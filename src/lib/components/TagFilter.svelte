<script lang="ts">
	...
	import { Tag as TagIcon, Check, ChevronDown, Rows3, Paintbrush } from 'lucide-svelte';
	...
	let { tags, selected = $bindable([]), mode = $bindable('all'), groupBy = $bindable(undefined), showTags = $bindable(undefined), showBands = $bindable(undefined), width = 'w-44' }: Props = $props();
...
<Popover.Root bind:open>
	...
	<Popover.Content class="w-56 p-1" align="start">
		...
		{/if}
		{#if groupBy !== undefined || showTags !== undefined}
